...
**Status:** Open
"Imagine you build the entire factory — the node SUBGRAPH is the optimizer. Where does it belong?" Plus: "maybe 100 nodes are a single purpose, like hierarchical sheets." Direction consistent with settled case law: storage stays FLAT (the graph-inside-a-node idea was ruled rendering-not-storage in the ADR-0018 discussion); a "sheet" = a purpose node (e.g. siemens-process) whose internals are IS_COMPONENT_OF children, collapsed/expanded as an LOD rendering unit (the family-bubble machinery generalized from taxonomy to composition). The OPTIMIZES edge attaches at the sheet handle. RULED (2026-08-09, the smelting expansion): **groups/sheets are a CONVENIENCE OVERLAY ONLY** — every node is full-level like every other node, forever; no second-class internals. Grouping is **emergent and retroactive**: "someone can come on later and say — hey, let's group these nodes as this process, because that's the emergent thing these clusters do." Nothing requires a group upfront; the sheet is a label applied to a cluster that already works. This is what keeps the forever-growing rule intact (technically-correct accepted now + easily editable later). Hub refinement (the Electric-Power-will-be-MASSIVE case): coarse hubs are legal and expected; when a hub needs splitting (grid power vs industrial MW vs low-voltage DC), the EXISTING moves apply — taxonomy children via extract_family/classify, per-consumer refinement via intercept/reconnect, inheritance for the shared parts — the same machinery that split 802.11 versions and extracted Smartphone. Still open: the overlay's storage shape (a `group` annotation? membership facts?), sheet-collapse rendering, sheet-level solve summaries.

## Q-27: The hub-split pipeline — "LLMs make node splitting tractable" (SPECIFIED, unbuilt)
**Status:** Open (user-specified 2026-08-09; composes existing machinery; needs the triage worker)
The founding thesis, stated explicitly: "This is why this project is only now possible. Splitting nodes used to be infeasible; LLMs make them tractable." The pipeline for splitting a massive hub (Electric Power → Electric Power + Grid-Level-Voltage …):
1. Split PROPOSED and voted (blast-radius-weighted — a hub's split affects its whole dependency mass, so the vote gate scales with it; ADR-0013 machinery).
2. Split executes: children created (extract_family/classify — existing verbs).
3. **Per-edge LLM triage**: every consumer edge of the old hub is classified to one of the children by an LLM (Tera/frontier-class) reading the edge's JUSTIFICATION + the children's DESCRIPTIONS — this is where the intent-on-its-face ruling (ADR-0045 §5, edge justifications) pays its dividend: the classifier never guesses intent, it reads it.
4. High-confidence → auto-reroute (reconnect verbs, old edges shadowed); low-confidence → DEFERS TO HUMAN JUDGMENT (decision tickets — existing).
5. Humans vote/keep/correct after the fact (challenges — existing).
Everything except the triage worker already exists; the worker is an agent workflow over MCP (get_edge → classify → verb), not new architecture. The economics: N-thousand-edge splits cost pennies of LLM time instead of человек-months — the tractability unlock the whole project rests on.

## Q-28: Retroactive universal wiring — the late-arriving ubiquitous dependency
**Status:** Open (raised 2026-08-09: "imagine you didn't have electricity for a long time — how do you add it to basically everything?")
The design discussion (first pass, to be attacked):
1. **Transitivity deflates "everything" to "the direct frontier."** Electricity does NOT need edges to every node — only to its DIRECT consumers (arc furnaces, electrolysis, lighting, fabs); the solver propagates dependency transitively to everything above them. The frightening version of the problem is partly an illusion.
2. **Families multiply each edge**: connect at the highest sensible ancestor (Electric Power → an "electronic device" family) and every instance PRESUMES it via ADR-0019 inheritance — one edge, thousands of consumers.
3. **Frontier discovery is the real work** — and it is the Q-27 shape again: an LLM sweep over candidates (semantic search proposes them; neighborhood embeddings make "who plausibly consumes electricity" a query), each hit born with a justification, low-confidence deferred to tickets, the whole campaign drip-posted as bounties ordered by blast radius.
4. THE WALKTHROUGH (2026-08-09, the 1M-node question "where do we begin"): (0) node arrives coarse, nothing scans; (1) discovery = semantic/lexical QUERY, O(K) candidates ranked similarity × blast radius; (2) lift candidates to LCAs first (ADR-0053) — a million nodes folds to tens of family-root attachment points; (3) campaign = drip bounties, biggest blast radius first; (4) Q-27 workers verdict each candidate ONCE — wire / skip-covered (closure query) / skip-unrelated / defer — EVERY VERDICT IS A MEMO (ADR-0054): closed bounties incl. skips are never re-asked; (5) steady state is free — the gate shows the hub to authors of new plausibly-related nodes at insert time (campaign handles stock, gate handles flow). Total: O(candidates) once + O(new) forever.
5. Open detail: the SKIP-MEMO's durable home — cheap version = fulfilled campaign bounty with the skip reason (current machinery, probably sufficient); rich version = first-class considered-record per pair. Lean cheap-until-it-hurts.
