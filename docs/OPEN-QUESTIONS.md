...
- **KNOWLEDGE_REQUIREMENT → collapsed into ENABLES**, refined by ADR-0025: the time gate is earliest-possible-date propagation through the *necessary layer only* — possibility traversal masks out works and people (their dates are contingent, not gating; the "1800 paper for no reason" guard). "Humans have no parents except knowledge" becomes a linter category rule (BIOLOGICAL_ENTITY only receives ENABLES from knowledge/concepts) validating recorded history, never constraining counterfactuals.
...
**Last open call (user's):** one story partition or two — split story-causality (INFLUENCES: drives-need, inhibits, precipitated, gave-rise-to, motivated-by, accelerates) from attribution (ASSOCIATION: authored, discovered, founded, custody, brand)? Partition-test argument for two: WWII-scale narrative fan-out where "what did this motivate?" and "who was involved?" are different common queries. Assistant lean: two — strengthened by the user recalling that institutional suppression (Catholic Church vs science) was the *original motivation* for inhibits: story causality is core content, likely deserving its own partition.
