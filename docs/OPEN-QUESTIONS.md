...
## Q-21: EdgeType reconciliation (TB-038)
The edge enum grew by accretion across two chats and five worked examples; one pass must produce the canonical vocabulary. Accumulated cargo: unreconciled draft edges from the old chats (SIGNIFICANTLY_IMPROVES, ACCELERATES_DEMAND, STIFLES, PROVIDES_RESOURCES, EXPLAINS_PRINCIPLE, CODIFIES, DESCRIBES_METHOD, FUNDED, CONTAINS, ADOPTS, DISCOVERED_USING) + new needs from this session (APPLIES_TO for brands, FOUNDED, SPUN_OFF_FROM, dated PRODUCED_BY custody for product lines crossing orgs). Also define per-type: which categories it may connect (`isValidConnection` linter), whether dependency traversal reads it, and whether it's narrative-layer (ghost edge).
