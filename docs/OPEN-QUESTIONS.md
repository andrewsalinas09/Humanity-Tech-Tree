...
A **reduction** pass, not a collection pass: map every existing enum member and every floated candidate (SIGNIFICANTLY_IMPROVES, ACCELERATES_DEMAND, STIFLES, PROVIDES_RESOURCES, EXPLAINS_PRINCIPLE, CODIFIES, DESCRIBES_METHOD, FUNDED, CONTAINS, ADOPTS, DISCOVERED_USING, APPLIES_TO, FOUNDED, SPUN_OFF_FROM, PRODUCED_BY…) onto the minimal orthogonal basis per ADR-0024's test (distinct type ⟺ distinct machine behavior), everything else becoming qualifiers. Straw-man basis: ENABLES / PART_OF / SUBTYPE / OPTIMIZES / SUCCEEDS / ASSOCIATION. To settle with the user: ingredient-vs-component (machine distinction or qualifier?), whether SUCCEEDS is machine-distinct from ASSOCIATION (rendering waves may count), whether KNOWLEDGE_REQUIREMENT's time-gate validation makes it ENABLES or its own behavior. Deliverables: final basis + old→new+qualifier mapping table + per-basis-type category-compatibility rules (isValidConnection linter) + starter qualifier vocabulary.
