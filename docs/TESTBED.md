...
**Stresses:** attribute absence semantics; whether lazy attribute vocabulary is safe. **Answer:** the **permissive-monotone default** (constraint example §Absence) — undeclared attribute → constraint passes as *presumed-satisfiable*, visibly labeled; never a failure (ADR-0015: incompleteness must survive) and strictly monotone (declaring can only prune more → ADR-0023 order-independence holds). The vocabulary is unbounded interned data (ADR-0004), declaration is demand-driven, and the debug-harness bounty loop (ADR-0025 §6) is the convergence engine: each absurd trace names the missing seam attribute, one added fact kills the class. **Status:** Solved (design).
