...
**Stresses:** created-in-error identities; append-only vs cleanliness; whether `flag` is enough. **Answer (open):** needs a tombstone mechanism that is not deletion — Q-23. Until then: flag the husk, exclude flagged-husk nodes from tiles? (rendering-level, reversible). **Status:** OPEN (Q-23).
