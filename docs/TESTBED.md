...
### TB-070 — The bibliography husk (node created in error, no merge target)
An agent, fulfilling a WANT_EVIDENCE request, created a WORK_PUBLICATION node for the Bardeen–Brattain patent purely to have a citable source (ADR-0045 now forbids this), and cited the transistor's `name` claim (vacuous). The citation is repairable (retract + re-cite a real claim, plain source id). The node is not: the log is append-only, there is no node deletion by design, and — unlike every duplicate — this husk has NO legitimate merge target (merging a patent into the transistor would be semantically false).
**Stresses:** created-in-error identities; append-only vs cleanliness; whether `flag` is enough. **Answer (open):** needs a tombstone mechanism that is not deletion — Q-23. Until then: flag the husk, exclude flagged-husk nodes from tiles? (rendering-level, reversible). **Status:** Solved (ADR-0047: admin-gated tombstone tickets — the husk was removed by the first real one; as-of reads still see it).
...
**Stresses:** attribute-vs-node boundary at scale; family explosion; search. **Status:** Solved (design — the existing case law composes; needs a worked example when steel enters the graph).

### TB-077 — The Spruce Pine problem (regional natural attributes)
The world's semiconductor crucibles depend heavily on naturally ultra-pure quartz from Spruce Pine, North Carolina; other deposits need an EXTRA purification step. Nature's as-found declarations must therefore be REGIONAL (quartz purity varies by deposit), and the solver/regional machinery (TB-031 RegionalAvailability) must show WHY one mine is critical: elsewhere, the same capability requires an additional lit optimizer.
**Stresses:** regional attribute values; criticality analysis ("what breaks if this mine floods" = blast radius over capabilities); nature-declaration honesty. **Status:** OPEN (design direction set in ADR-0052 round 2; regional values unimplemented).
