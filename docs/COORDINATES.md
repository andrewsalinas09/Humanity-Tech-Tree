...
- **Versions are satellites, not citizens** (user ruling, v1.2 — 2026-08-08): a node with an `IS_REFINEMENT_OF` edge to a family never enters the world layout (before this rule, 802.11b landed among the *foundations* because it has no providers of its own). Versions attach to their family root as a **timeline cascade — down and to the right**, ordered by the refine edge's `start_date` (b→g→n→ax reads as a waterfall of generations). Collapsed by default in the viewer; the family card carries the *demerge* toggle (ADR-0018 family-bubble tier, first mechanism). Version nodes carry `version`/`family` props; any edge touching a version carries `vfamily`, so the whole satellite system hides and reveals together.
