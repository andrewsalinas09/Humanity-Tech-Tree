...
## The authoring loop (what using it feels like)

Ask an agent "add iPhone." It lands in seconds — a coarse, true skeleton, glowing red for `[needs citation]`. You dig the CPU rabbit hole; a stranger digs WiFi; an agent fleet digs batteries — simultaneously, colliding never (any order, same graph). Everything lands red-first, then solidifies: sources attach, machine verifiers lift, humans confirm, the community ratifies — red ripening to green as verification catches up to generation. The graph never shrinks, only grows; nothing is ever right-the-first-time, only true-at-some-resolution and refined forever. Hyper-parallelizable by design: hundreds of nodes an hour through an API whose architecture absorbs all the complexity, so contributing feels like play while the result stays research-grade. Watching the red turn green is watching humanity verify its own knowledge in real time.
