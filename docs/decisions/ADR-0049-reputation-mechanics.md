...
7. **Challenges** (per the same-day rulings): `challenge.open {subject, grounds, remedy: [verb calls]}`, `challenge.vote {support, reason}`, `challenge.resolve {outcome}` — all facts. Votes are reputation-weighted and ADVISORY; **an admin ratifies every passed challenge**; on upheld, the pre-staged remedy verbs execute under the resolver's identity with the challenge as provenance. Tombstone remedies are legal INSIDE a challenge: the ratifying admin IS ADR-0047's gate, so `{verb: tombstone}` in a remedy applies the tombstone directly with the challenge as provenance. Delete-requests remain the lightweight lane for unambiguous removals; the challenge is the general instrument — wrongness + staged fix + votes + ratification.
